...
def gestionar_cuenta(request, cuenta_id=None):
    ...
    if cuenta_id and not request.user.is_admin():
...
def lista_cuentas(request):
    ...
    # aceptar tanto 'q' (template) como 'search' (posible variantes)
    query = request.GET.get('q', request.GET.get('search', '')).strip()
    cuentas_qs = Cuenta.objects.all()
    
    # Validar y escapar la búsqueda; buscar por nombre (first/last), email o username
